...
if(but.equals("Modificar"))
	...
	<script>self.location='receta_col_surtir.jsp?id_prov=<%=eliminar_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=&descrip1=&present1=&indica1=<%=indica1_jv%>&sol1=&sur1=&cause=<%=cause_jv%>&foliore=<%=foliore_jv%>&encar=<%=request.getParameter("encar")%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&servicio=<%=request.getParameter("servicio")%>&encarser=<%=encarser_jv%>&aa5=<%=date2%>&part='</script>
	<%
/*
      ...
      <table width="710" height="45" border="0" align="center" cellpadding="2">
          ...
          <td height="123" colspan="3" class="style4"><table width="746" border="0" align="center" cellpadding="2">
            ...
            <tr>
              ...
              <td colspan="2" class="style2">EXIST&nbsp;
                <input type="text" name="txtf_exist" id="txtf_exist" size="1"  class="style2" value="<%=cant_jv%>" onKeyPress="return handleEnter(this, event)" readonly="true"/>
			...
			while (rset1.next())
	   {
            ...
            <tr>
			  ...
			  <%}%>
              <td class="style11"><input name="Submit" type="submit" class="but" value="Modificar" onChange="setSur1(this.form)"  onClick="return existencia0()"/>&nbsp; <a href="javascript:history.back(1)">Regresar</a></td>
              <td class="style11">&nbsp;</td>
            </tr>
